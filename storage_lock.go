...
func NewStorageLock(storage storage.Storage, options ...*StorageLockOptions) *StorageLock {
	...
	if option.LockId == "" {
		...
		e.SetLockId(option.LockId).AppendActionByName("random-lock-id")
	}
...
func (x *StorageLock) Lock(ctx context.Context, ownerId ...string) error {
	...
	// 触发一个获取锁的事件
	e := events.NewEvent(x.options.LockId).SetType(events.EventTypeLock).SetListeners(x.options.EventListeners)

	for {

		err := x.lockWithRetry(ctx, e.Fork(), ownerId...)
		if err == nil {
			e.AppendActionByName("success").Publish(ctx)
			return nil
		}

		// TODO 2023-6-21 00:33:21 错误处理
		e.Fork().AppendActionByName("error").SetErr(err).Publish(ctx)

		select {
		case <-ctx.Done():
			e.AppendActionByName("timeout").Publish(ctx)
			return err
		case <-time.After(time.Microsecond):
			e.Fork().AppendActionByName("sleep-retry").Publish(ctx)
...
func (x *StorageLock) lockWithRetry(ctx context.Context, e *events.Event, ownerId ...string) error {
	...
	e.AppendActionByName("lock-with-retry")

	// 如果没有指定ownerId，则为其生成一个默认的ownerId
	if len(ownerId) == 0 {
		ownerId = append(ownerId, x.ownerIdGenerator.getDefaultOwnId())
		e.AppendActionByName("use-default-owner")
	} else if len(ownerId) >= 2 {
		e.AppendActionByName("owner-error").Publish(ctx)
	...
	if err != nil && !errors.Is(err, ErrLockNotFound) {
		e.AppendActionByName("get-lock-information-error").SetErr(err).Publish(ctx)
		return err
	}

	// 如果锁的信息存在，则说明之前锁就已经存在了
	if lockInformation != nil {
		e.AppendActionByName("to-lock-exists").Publish(ctx)
		return x.lockExists(ctx, e.Fork(), ownerId[0], lockInformation)
	} else {
		// 否则认为之前锁是不存在的
		e.AppendActionByName("to-lock-not-exists").Publish(ctx)
...
func (x *StorageLock) reentryLock(ctx context.Context, e *events.Event, ownerId string, lockInformation *storage.LockInformation) error {
	...
	e.AppendActionByName("reentry").SetLockInformation(lockInformation)

	// 计算从当前时间开始计算的租约的过期时间
	expireTime, err := x.getLeaseExpireTime(ctx, e)
	if err != nil {
		e.AppendActionByName("get-lease-expire-time-error").SetErr(err).Publish(ctx)
		return err
	}
	...
	if err == nil {
		e.AppendActionByName("update-with-version-success").Publish(ctx)
		return nil
	}
	// 如果发生了错误，除非是版本未命中的错误，否则都不再重试了，直接认为是中断式的错误
	if !errors.Is(err, ErrVersionMiss) {
		e.AppendActionByName("update-with-version-error").SetErr(err).Publish(ctx)
		return err
	}
	e.AppendActionByName("update-with-version-miss")

	// 执行到这里，如果没更新成功，并且还有重试次数的话则重试
	select {
	case <-time.After(time.Microsecond):
		// 还有时间，再重试一次
		e.AppendActionByName("to-lock-with-retry").Publish(ctx)
...
func (x *StorageLock) cleanExpiredLockAndRetry(ctx context.Context, e *events.Event, ownerId string, lockInformation *storage.LockInformation) error {
	...
	e.AppendActionByName("clean-expired-lock-and-retry").SetLockInformation(lockInformation)
	...
	if err != nil {
		e.AppendActionByName("get-storage-time-error").SetErr(err).Publish(ctx)
		return err
	}
	...
	if lockInformation.LeaseExpireTime.After(storageTime) {
		...
		e.AppendActionByName("lock-by-other-and-not-expired").Publish(ctx)
		return ErrLockFailed
	}
	e.AppendActionByName("other-lock-expired")
	...
	if err != nil {
		e.Fork().SetLockInformation(lockInformation).AppendActionByName("delete-with-version-error").SetErr(err).Publish(ctx)
	} else {
		e.Fork().SetLockInformation(lockInformation).AppendActionByName("delete-with-version-success").Publish(ctx)
	}
...
func (x *StorageLock) lockNotExists(ctx context.Context, e *events.Event, ownerId string, lockInformation *storage.LockInformation) error {
	...
	e.AppendActionByName("lock-not-exists").SetLockInformation(lockInformation)
	...
	if err != nil {
		...
		e.Fork().SetLockInformation(lockInformation).AppendActionByName("insert-with-version-error").SetErr(err).Publish(ctx)
		...
		case <-ctx.Done():
			...
			return ErrLockFailed
		case <-time.After(time.Microsecond):
			e.AppendActionByName("retry").Publish(ctx)
			return x.lockWithRetry(ctx, e.Fork(), ownerId)
		}
	...
	if x.storageLockWatchDog != nil {
		e.Fork().SetLockInformation(lockInformation).AppendActionByName("stop-watch-dog").SetWatchDogId(x.storageLockWatchDog.GetID()).Publish(ctx)
	...
	e.Fork().SetLockInformation(lockInformation).AppendActionByName("start-watch-dog").SetWatchDogId(x.storageLockWatchDog.GetID()).Publish(ctx)
...
func (x *StorageLock) getLeaseExpireTime(ctx context.Context, e *events.Event) (time.Time, error) {
	...
	storageTime, err := x.storage.GetTime(ctx)
	e.Fork().AppendAction(getTimeAction.End().SetErr(err).SetPayload(storageTime.String())).Publish(ctx)
...
func (x *StorageLock) UnLock(ctx context.Context, ownerId ...string) error {
	...
	for {
		...
		if err == nil {
			e.AppendActionByName("unlock-success").Publish(ctx)
			return nil
		}

		// TODO 2023-6-21 00:33:21 错误处理
		e.Fork().AppendActionByName("unlock-error").SetErr(err).Publish(ctx)
...
func (x *StorageLock) unlockWithRetry(ctx context.Context, e *events.Event, ownerId ...string) error {
	...
	e.AppendActionByName("unlock-with-retry")

	// 如果没有指定ownerId的话，则为其生成一个默认的ownerId
	if len(ownerId) == 0 {
		e.AppendActionByName("with-default-owner-id")
		ownerId = append(ownerId, x.ownerIdGenerator.getDefaultOwnId())
	} else if len(ownerId) >= 2 {
		e.AppendActionByName("owner-id-set-error").AppendActionByName(strings.Join(ownerId, ", ")).Publish(ctx)
		return ErrOwnerCanOnlyOne
	}
	...
	if lockInformation == nil {
		e.AppendActionByName("lock-information-not-exists").Publish(ctx)
		return ErrLockNotFound
	}

	// 如果锁的当前持有者的ID不是自己，则无权释放锁
	if lockInformation.OwnerId != ownerId[0] {
		e.AppendActionByName("not-lock-owner").Publish(ctx)
...
func (x *StorageLock) reentryUnlock(ctx context.Context, e *events.Event, ownerId string, lockInformation *storage.LockInformation, lastVersion storage.Version) error {
	...
	e.AppendActionByName("reentry-unlock").SetLockInformation(lockInformation)
	...
	e.Fork().AppendAction(updateAction.End().SetErr(err).SetPayload(lockInformation.ToJsonString())).Publish(ctx)
...
func (x *StorageLock) unlockWithClean(ctx context.Context, e *events.Event, ownerId string, lockInformation *storage.LockInformation, lastVersion storage.Version) error {
	...
	e.AppendActionByName("unlockWithClean").SetLockInformation(lockInformation)
	...
	e.Fork().AppendAction(deleteAction.End().SetErr(err).SetPayload(lockInformation.ToJsonString())).Publish(ctx)

	// 如果删除的时候遇到错误，则直接认为锁释放失败
	if err != nil {

		e.AppendActionByName("storage-delete-with-version-error")

		if errors.Is(err, ErrVersionMiss) {

			e.AppendActionByName("is-version-miss-error")
			...
			case <-ctx.Done():
				...
				return ErrLockFailed
			case <-time.After(time.Microsecond):
				e.AppendActionByName("to-unlock-with-retry").Publish(ctx)
				return x.unlockWithRetry(ctx, e.Fork(), ownerId)
			}

		} else {
			e.AppendActionByName("is-other-error").SetErr(err).Publish(ctx)
			return err
		}
	}

	// 执行到这里表示已经删除成功了，则将租约续期的协程停掉
	e.Fork().AppendActionByName("stop-watch-dog").SetWatchDogId(x.storageLockWatchDog.GetID()).Publish(ctx)
	x.storageLockWatchDog.Stop()

	e.AppendActionByName("done").Publish(ctx)
	return nil
}
...
func (x *StorageLock) getLockInformation(ctx context.Context, e *events.Event) (*storage.LockInformation, error) {
	...
	e.Fork().AppendAction(getAction.End().SetErr(err).SetPayload(lockInformationJsonString)).Publish(ctx)
