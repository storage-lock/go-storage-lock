...
func NewStorageLock(storage storage.Storage, options ...*StorageLockOptions) *StorageLock {
	...
	// 触发创建锁的事件
	e := event.NewEvent(option.LockId).SetType(event.EventTypeCreateLock).SetStorageName(storage.GetName()).SetEventListeners(option.EventListeners)

	// 如果没有设置锁的ID的话，则为其生成一个随机的默认的ID，但是通常情况下不是最佳实践，应该避免这种用法
	if option.LockId == "" {
		option.LockId = strings.ReplaceAll(uuid.New().String(), "-", "")
		e.SetLockId(option.LockId).AppendAction("random-lock-id")
	}
...
func (x *StorageLock) Lock(ctx context.Context, ownerId ...string) error {
	...
	// 触发一个获取锁的事件
	e := event.NewEvent(x.options.LockId).SetType(event.EventTypeLock).SetEventListeners(x.options.EventListeners)

	for {

		err := x.lockWithRetry(ctx, e.Fork(), ownerId...)
		if err == nil {
			e.AppendAction("success").Publish(ctx)
			return nil
		}

		// TODO 2023-6-21 00:33:21 错误处理
		e.Fork().AppendAction("error").SetErr(err).Publish(ctx)

		select {
		case <-ctx.Done():
			e.AppendAction("timeout").Publish(ctx)
			return err
		case <-time.After(time.Microsecond):
			e.Fork().AppendAction("sleep-retry").Publish(ctx)
...
func (x *StorageLock) lockWithRetry(ctx context.Context, e *event.Event, ownerId ...string) error {

	e.AppendAction("lock-with-retry")

	// 如果没有指定ownerId，则为其生成一个默认的ownerId
	if len(ownerId) == 0 {
		ownerId = append(ownerId, x.ownerIdGenerator.getDefaultOwnId())
		e.AppendAction("use-default-owner")
	} else if len(ownerId) >= 2 {
		e.AppendAction("owner-error").Publish(ctx)
	...
	if err != nil && !errors.Is(err, ErrLockNotFound) {
		e.AppendAction("get-lock-information-error").SetErr(err).Publish(ctx)
		return err
	}

	// 如果锁的信息存在，则说明之前锁就已经存在了
	if lockInformation != nil {
		e.AppendAction("to-lock-exists").Publish(ctx)
		return x.lockExists(ctx, e.Fork(), ownerId[0], lockInformation)
	} else {
		// 否则认为之前锁是不存在的
		e.AppendAction("to-lock-not-exists").Publish(ctx)
		return x.lockNotExists(ctx, e.Fork(), ownerId[0], lockInformation)
	}
}

// 尝试获取已经存在的锁
func (x *StorageLock) lockExists(ctx context.Context, e *event.Event, ownerId string, lockInformation *storage.LockInformation) error {

	e.AppendAction("lock-exists")

	// 给定的锁已经存在了，又分为两种情况，一种是锁就是自己持有的，一种是锁被别人持有
	if lockInformation.OwnerId == ownerId {
		e.AppendAction("i-am-owner").Publish(ctx)
		return x.reentryLock(ctx, e.Fork(), ownerId, lockInformation)
	} else {
		e.AppendAction("i-am-not-owner").Publish(ctx)
		return x.cleanExpiredLockAndRetry(ctx, e.Fork(), ownerId, lockInformation)
	}
}

// 进入重入锁的逻辑，尝试对可重入锁的层级加一
func (x *StorageLock) reentryLock(ctx context.Context, e *event.Event, ownerId string, lockInformation *storage.LockInformation) error {

	e.AppendAction("reentry").SetLockInformation(lockInformation)

	// 计算从当前时间开始计算的租约的过期时间
	expireTime, err := x.getLeaseExpireTime(ctx)
	if err != nil {
		e.AppendAction("get-lease-expire-time-error").SetErr(err).Publish(ctx)
		return err
	}
	...
	if err == nil {
		e.AppendAction("update-with-version-success").Publish(ctx)
		return nil
	}
	// 如果发生了错误，除非是版本未命中的错误，否则都不再重试了，直接认为是中断式的错误
	if !errors.Is(err, ErrVersionMiss) {
		e.AppendAction("update-with-version-error").SetErr(err).Publish(ctx)
		return err
	}
	e.AppendAction("update-with-version-miss")

	// 执行到这里，如果没更新成功，并且还有重试次数的话则重试
	select {
	case <-time.After(time.Microsecond):
		// 还有时间，再重试一次
		e.AppendAction("to-lock-with-retry").Publish(ctx)
...
func (x *StorageLock) cleanExpiredLockAndRetry(ctx context.Context, e *event.Event, ownerId string, lockInformation *storage.LockInformation) error {

	e.AppendAction("clean-expired-lock-and-retry").SetLockInformation(lockInformation)
	...
	if err != nil {
		e.AppendAction("get-storage-time-error").SetErr(err).Publish(ctx)
		return err
	}
	...
	if lockInformation.LeaseExpireTime.After(storageTime) {
		...
		e.AppendAction("lock-by-other-and-not-expired").Publish(ctx)
		return ErrLockFailed
	}
	e.AppendAction("other-lock-expired")
	...
	if err != nil {
		e.Fork().SetLockInformation(lockInformation).AppendAction("delete-with-version-error").SetErr(err).Publish(ctx)
	} else {
		e.Fork().SetLockInformation(lockInformation).AppendAction("delete-with-version-success").Publish(ctx)
	}
...
func (x *StorageLock) lockNotExists(ctx context.Context, e *event.Event, ownerId string, lockInformation *storage.LockInformation) error {

	e.AppendAction("lock-not-exists").SetLockInformation(lockInformation)
	...
	if err != nil {
		...
		e.Fork().SetLockInformation(lockInformation).AppendAction("insert-with-version-error").SetErr(err).Publish(ctx)
		...
		case <-ctx.Done():
			...
			return ErrLockFailed
		case <-time.After(time.Microsecond):
			e.AppendAction("retry").Publish(ctx)
			return x.lockWithRetry(ctx, e.Fork(), ownerId)
		}
	...
	if x.storageLockWatchDog != nil {
		e.Fork().SetLockInformation(lockInformation).AppendAction("stop-watch-dog").SetWatchDogId(x.storageLockWatchDog.GetID()).Publish(ctx)
		x.storageLockWatchDog.Stop()
	}

	// 启动一个新的租约续期协程
	x.storageLockWatchDog = NewStorageLockWatchDog(x, ownerId)
	x.storageLockWatchDog.Start()
	e.Fork().SetLockInformation(lockInformation).AppendAction("start-watch-dog").SetWatchDogId(x.storageLockWatchDog.GetID()).Publish(ctx)
...
// 获取租约下一次的过期时间
func (x *StorageLock) getLeaseExpireTime(ctx context.Context, e *event.Event) (time.Time, error) {

	getTimeEvent := e.Fork().AppendAction(event.ActionStorageGetTime)
	storageTime, err := x.storage.GetTime(ctx)
	getTimeEvent.SetErr(err).Publish(ctx)
...
func (x *StorageLock) UnLock(ctx context.Context, ownerId ...string) error {
	...
	e := event.NewEvent(x.options.LockId).SetType(event.EventTypeUnlock).SetStorageName(x.storage.GetName()).SetEventListeners(x.options.EventListeners)

	for {

		err := x.unlockWithRetry(ctx, e.Fork(), ownerId...)
		if err == nil {
			e.AppendAction("unlock-success").Publish(ctx)
			return nil
		}

		// TODO 2023-6-21 00:33:21 错误处理
		e.Fork().AppendAction("unlock-error").SetErr(err).Publish(ctx)
...
func (x *StorageLock) unlockWithRetry(ctx context.Context, e *event.Event, ownerId ...string) error {

	e.AppendAction("unlock-with-retry")

	// 如果没有指定ownerId的话，则为其生成一个默认的ownerId
	if len(ownerId) == 0 {
		e.AppendAction("with-default-owner-id")
		ownerId = append(ownerId, x.ownerIdGenerator.getDefaultOwnId())
	} else if len(ownerId) >= 2 {
		e.AppendAction("owner-id-set-error").AppendAction(strings.Join(ownerId, ", ")).Publish(ctx)
		return ErrOwnerCanOnlyOne
	}
	...
	if lockInformation == nil {
		e.AppendAction("lock-information-not-exists").Publish(ctx)
		return ErrLockNotFound
	}

	// 如果锁的当前持有者的ID不是自己，则无权释放锁
	if lockInformation.OwnerId != ownerId[0] {
		e.AppendAction("not-lock-owner").Publish(ctx)
	...
	if lockInformation.LockCount > 0 {
		e.AppendAction("to-reentry-unlock").Publish(ctx)
		return x.reentryUnlock(ctx, e.Fork(), ownerId[0], lockInformation, lastVersion)
	} else {
		// 如果经过这次操作之后锁的锁的锁定次数为0，说明应该彻底释放掉这个锁了，将其从Storage中清除
		e.AppendAction("to-unlock-with-clean").Publish(ctx)
		return x.unlockWithClean(ctx, e.Fork(), ownerId[0], lockInformation, lastVersion)
	}
}

// 可重入锁的层级减一，但是并没有彻底释放，更新数据库中的锁的信息
func (x *StorageLock) reentryUnlock(ctx context.Context, e *event.Event, ownerId string, lockInformation *storage.LockInformation, lastVersion storage.Version) error {

	e.AppendAction("reentry-unlock").SetLockInformation(lockInformation)
...
func (x *StorageLock) unlockWithClean(ctx context.Context, e *event.Event, ownerId string, lockInformation *storage.LockInformation, lastVersion storage.Version) error {

	e.AppendAction("unlock-with-clean").SetLockInformation(lockInformation)

	// 重入锁的次数已经被释放干净了，现在需要将其彻底删除
	deleteEvent := e.Fork().AppendAction("storage-delete-with-version")
	err := x.storage.DeleteWithVersion(ctx, x.options.LockId, lastVersion, lockInformation)
	deleteEvent.SetErr(err).Publish(ctx)

	// 如果删除的时候遇到错误，则直接认为锁释放失败
	if err != nil {

		e.AppendAction("storage-delete-with-version-error")

		if errors.Is(err, ErrVersionMiss) {

			e.AppendAction("is-version-miss-error")
			...
			case <-ctx.Done():
				...
				return ErrLockFailed
			case <-time.After(time.Microsecond):
				e.AppendAction("to-unlock-with-retry").Publish(ctx)
				return x.unlockWithRetry(ctx, e.Fork(), ownerId)
			}

		} else {
			e.AppendAction("is-other-error").SetErr(err).Publish(ctx)
			return err
		}
	}

	// 执行到这里表示已经删除成功了，则将租约续期的协程停掉
	e.Fork().AppendAction("stop-watch-dog").SetWatchDogId(x.storageLockWatchDog.GetID()).Publish(ctx)
	x.storageLockWatchDog.Stop()

	e.AppendAction("done").Publish(ctx)
	return nil
}
...
// 获取之前的锁保存的信息
func (x *StorageLock) getLockInformation(ctx context.Context, e *event.Event) (*storage.LockInformation, error) {

	getEvent := e.Fork().AppendAction(event.ActionStorageGet)
	...
	return storage.FromJsonString(lockInformationJsonString)
}
